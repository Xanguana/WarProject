...
public class Jogador {
	...
	public void definirExercito(int nro_soldados) {
		...
		for (int i=0;i<paises.size();i++) {
			System.out.println((i+1)+")"+paises.get(i).getNome()+"["+paises.get(i).getExercito().get(0).getQuantidade()+"] ");
		}
	...
	public void ataque() {
		...
		int atacante = scan.nextInt();atacante-=1;
		System.out.println("Escolha um alvo entre os abaixos: ");
		for(int j=0;j<paises.get(atacante).getVisinhos().size();j++) {
			if(paises.get(atacante).getVisinhos().get(j).getJogador().getNome() != paises.get(atacante).getJogador().getNome()) {
		...
		int opcaoExercito = scan.nextInt(); opcaoExercito-=1;
		System.out.println("Com quantos "+paises.get(atacante).getExercito().get(opcaoExercito).getTipo()+" pretende atacar:");
		int soldadosAtacantes = scan.nextInt();
		if(soldadosAtacantes > paises.get(atacante).getExercito().get(opcaoExercito).getQuantidade()) {
			System.out.println("Voce não tem essa força de ataque, tente outra vez com um numero menor que o total dos seus soldados");
			ataque();
		}else {
